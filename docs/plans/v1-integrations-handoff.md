...
### 4.5 Simplify-pass pending items
(See the simplify commit message on this branch for what was applied. Anything it listed but left unapplied — e.g. optional shared `TrendCard`/empty-state component extractions on the frontend, optional `_get_project_or_404` consolidation into `app/api/deps.py` — is optional polish, not required.)
