...
**Ready-made, runnable implementations of the stub provider, pre-import launcher, seeds, the Group A harness, and the full B–H test plan are committed under [`testing/local-stack/`](../../testing/local-stack/README.md)** — follow its README bring-up order. The steps below are kept as the reference spec for how that tooling works.
...
The full case-level spec (exact expected values, worker-safety rules, execution-order constraints) is committed at `testing/local-stack/test-plan-v1-integrations-traffic-analytics.md`; summary:
